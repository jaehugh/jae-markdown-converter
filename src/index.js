...
<body><div class="shell">
  ...
  <main>
    ...
    <section class="work">
      <div class="input"><label class="drop" id="drop"><input id="file" type="file" accept=".pdf,.docx,.html,.htm,.json,.csv,.xml,.txt,application/pdf,text/*"><div><b>Drop a document here</b><span>or choose a local file</span></div></label><div class="types"><span>PDF</span><span>DOCX</span><span>HTML</span><span>JSON</span><span>CSV</span><span>XML</span><span>TXT</span></div><p class="status" id="status">Ready. No fixed file-size limit; large PDFs depend on your browser's memory.</p></div>
...
<script type="module">
  const drop=document.querySelector("#drop"),fileInput=document.querySelector("#file"),status=document.querySelector("#status"),output=document.querySelector("#output"),copy=document.querySelector("#copy"),download=document.querySelector("#download"),clear=document.querySelector("#clear");
  let name="document";
  ...
  clear.onclick=()=>{output.value="";fileInput.value="";enabled(false);message("Ready. No fixed file-size limit; large PDFs depend on your browser's memory.")};copy.onclick=async()=>{await navigator.clipboard.writeText(output.value);copy.textContent="Copied";setTimeout(()=>copy.textContent="Copy Markdown",1000)};download.onclick=()=>{const url=URL.createObjectURL(new Blob([output.value],{type:"text/markdown"})),a=document.createElement("a");a.href=url;a.download=name+".md";a.click();URL.revokeObjectURL(url)}
</script></body></html>`;
